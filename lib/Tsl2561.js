...
class Tsl2561 {
    ...
    init(busNumber,deviceAddress=DEFAULT_ADDRESS){
        ow(busNumber,ow.number.greaterThanOrEqual(0))
        ow(deviceAddress, ow.number)
    ...
    async setIntegrationTime(value){
        ow(value,ow.number.inRange(0,3))
        this.$checkInit()
        value &= 0x01
        let current = await this.$readRegister(REGISTERS.TIMING)
        await (new Promise((resolve, reject) => {
    ...
    $readRegister(registerAddr,count=1){
        ow(registerAddr,ow.number)
        ow(count,ow.number.positive)
